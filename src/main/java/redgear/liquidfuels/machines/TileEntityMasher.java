...
public class TileEntityMasher extends TileEntityElectricMachine{
    ...
    public TileEntityMasher(){
        ...
        AdvFluidTank water = new AdvFluidTank(FluidContainerRegistry.BUCKET_VOLUME * 4);
        AdvFluidTank biomass = new AdvFluidTank(FluidContainerRegistry.BUCKET_VOLUME * 4);
        
        waterTank = addTank(water, 11, 13, 16, 60);
        biomassTank = addTank(biomass, 149, 13, 16, 60);
        
        water.setPressure(-1);
        biomass.setPressure(1);
        
        water.addFluidMap(FluidRegistry.WATER);
    ...
    protected void checkWork(){
		...
		for(int i = 0; i <= 3; i++){
			...
			currRecipe = RecipeManager.masherRegistry.getMasherRecipe(stack);
			
			if (currRecipe != null && getTank(waterTank).getFluidAmount() > currRecipe.water && (getTank(biomassTank).isEmpty() || getTank(biomassTank).contains(currRecipe.output)) && getTank(biomassTank).getSpace() > currRecipe.amount){
				addWork(currRecipe.work, currRecipe.power);
				decrStackSize(i, 1);
				getTank(waterTank).drain(currRecipe.water, true);// use water
				output = new FluidStack(currRecipe.output, currRecipe.amount);
				return;
			}
    ...
    protected void doPostWork() {
    	if(output != null)
    		getTank(biomassTank).fillOverride(output, true);
    	output = null;
    }

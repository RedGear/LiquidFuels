...
public class TileEntityBoiler extends TileEntityElectricMachine {
	...
	private final int powerRatio = 48;
	...
	public TileEntityBoiler() {
		...
		waterTank.addFluidMap(FluidRegistry.WATER);
		waterTank.setPressure(-1);
	...
	protected void checkWork(){
		rate = (int) Math.min(getEnergyAmount() / powerRatio, Math.min(steamTank.getSpace() / steamRatio, waterTank.getFluidAmount()));
	...
	protected void doPostWork(){
		steamTank.fillOverride(new FluidStack(LiquidFuels.steamFluid, steamRatio * rate), true);

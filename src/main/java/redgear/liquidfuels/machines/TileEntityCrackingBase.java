...
public class TileEntityCrackingBase extends TileEntityElectricMachine {
	...
	private static final FluidStack[] outputMap = {
		new FluidStack(LiquidFuels.asphaltFluid, 2),
		new FluidStack(LiquidFuels.petroleumCokeFluid, 5),
		new FluidStack(LiquidFuels.dieselFluid, 10),
		new FluidStack(LiquidFuels.keroseneFluid, 5),
		new FluidStack(LiquidFuels.gasolineFluid, 20),};
	...
	public TileEntityCrackingBase(){
        ...
        oilTank.addFluidMap(LiquidFuels.oilFluid);
        oilTank.setPressure(-1);
	...
	protected void checkWork() {
		if(checkMulitBlock() && oilTank.getFluidAmount() >= oilRate && steamTank.getFluidAmount() >= steamRate){
			for(int i = 0; i <= outputMap.length - 1; i++)
				if(!(((TileEntityCrackingTower) worldObj.getBlockTileEntity(xCoord, yCoord + 1 + i, zCoord)).getTank(0).getSpace() >= outputMap[i].amount))
					return;
	...
	protected void doPostWork() {
		for(int i = 0; i <= outputMap.length - 1; i++)
			((TileEntityCrackingTower) worldObj.getBlockTileEntity(xCoord, yCoord + 1 + i, zCoord)).getTank(0).fillOverride(outputMap[i], true);	
	}
